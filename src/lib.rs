...
bitflags! {
    ...
    pub struct EPolyFlags: u32 {
        ...
        const NOT_SOLID		= 0x00000008;	// Poly is not solid, doesn't block.
        const ENVIRONMENT   = 0x00000010;	// Poly should be drawn environment mapped.
        const SEMI_SOLID	= 0x00000020;	// Poly is semi-solid = collision solid, Csg nonsolid.
        const MODULATED 	= 0x00000040;	// Modulation transparency.
        const FAKE_BACKDROP	= 0x00000080;	// Poly looks exactly like backdrop.
        const TWO_SIDED     = 0x00000100;	// Poly is visible from both sides.
        const NO_SMOOTH		= 0x00000800;	// Don't smooth textures.
        const ALPHA_TEXTURE = 0x00001000;	// Honor texture alpha (reuse BigWavy and SpecialPoly flags)
        const FLAT			= 0x00004000;	// Flat surface.
        const NO_MERGE		= 0x00010000;	// Don't merge poly's nodes before lighting when rendering.
        const NO_Z_TEST		= 0x00020000;	// Don't test Z buffer
        const ADDITIVE		= 0x00040000;	// sjs - additive blending, (Aliases DirtyShadows).
        const SPECIAL_LIT	= 0x00100000;	// Only speciallit lights apply to this poly.
        const WIREFRAME		= 0x00200000;	// Render as wireframe
        const UNLIT			= 0x00400000;	// Unlit.
        const PORTAL		= 0x04000000;	// Portal between iZones.
        const ANTI_PORTAL   = 0x08000000;	// Antiportal
        ...
        // Internal.
        const ED_PROCESSED 	= 0x40000000;	// FPoly was already processed in editorBuildFPolys.
        const ED_CUT       	= 0x80000000;	// FPoly has been split by SplitPolyWithPlane.
...
impl FPoly {
            ...
            if let Some(front_poly) = front_poly {
                if let Some(back_poly) = back_poly {
                    ...
                    *front_poly = self.clone();
                    front_poly.poly_flags |= EPolyFlags::ED_CUT;
                    front_poly.vertices.clear();

                    *back_poly = self.clone();
                    back_poly.poly_flags |= EPolyFlags::ED_CUT;
    ...
    pub fn split_in_half(&mut self) -> Self {
        ...
        self.poly_flags |= EPolyFlags::ED_CUT;
        other_half.poly_flags |= EPolyFlags::ED_CUT;
...
pub struct FBspNode {
    ...
    pub back_node_index: i32,   // Index to the node in front (in the direction of the normal) (union with child_index)??
    pub front_node_index: i32,  // Index to the node in back (opposite direction of the normal)
...
bitflags! {
    ...
    pub struct EBspNodeFlags : u8 {
        const NONE = 0;
        const NOT_CSG = 0x0001; // Node is not a Csg splitter, i.e. is a transparent poly.
        const SHOOT_THROUGH = 0x0002; // Can shoot through (for projectile solid ops).
        const NOT_VIS_BLOCKING = 0x0004; // Node does not block visibility, i.e. is an invisible collision hull.
        const POLY_OCCLUDED = 0x0008; // Node's poly was occluded on the previously-drawn frame.
        const BOX_OCCLUDED = 0x0010; // Node's bounding box was occluded.
        const BRIGHT_CORNERS = 0x0010; // Temporary.
        const IS_NEW = 0x0020; // Editor: Node was newly-added.
        const IS_FRONT = 0x0040; // Filter operation bounding-sphere precomputed and guaranteed to be front.
        const IS_BACK = 0x0080; // Guaranteed back.
...
impl FBspNode {
    ...
    pub fn is_csg(&self, extra_flags: EBspNodeFlags) -> bool {
        self.vertex_count > 0 && (self.node_flags & (EBspNodeFlags::IS_NEW | EBspNodeFlags::NOT_CSG | extra_flags)) == EBspNodeFlags::NONE
    }
...
pub struct FCoplanarInfo {
    original_node_index: Option<i32>,
	back_node_index: Option<i32>,
...
pub fn filter_ed_poly(filter_func: BspFilterFunc, model: &UModel, node_index: usize, ed_poly: &FPoly, coplanar_info: FCoplanarInfo, outside: bool) {
    ...
    let mut our_front_node_index = 0;
    let mut our_back_node_index = 0;
...
impl BspBuilder {
    ...
    pub fn bsp_brush_csg(&mut self, actor: Rc<ABrush>, model: &mut UModel, poly_flags: EPolyFlags, csg_operation: ECsgOper, build_bounds: i32, merge_polygons: bool) {
        ...
        if csg_operation != ECsgOper::ADD {
            poly_flags_mask |= EPolyFlags::SEMI_SOLID | EPolyFlags::NOT_SOLID;
        }
        ...
        for i in 0..brush.polys.len() {
            ...
            ed_poly.poly_flags &= !EPolyFlags::ED_CUT;
